...
void tst_simpletransaction::transactionAdd() {
  ...
  Enums::PROCESS firstResult = st.transactionIsOver(Enums::PASS_SHOW);
  Enums::PROCESS secondResult = st.transactionIsOver(Enums::GIT_PULL);
  QVERIFY2(firstResult == Enums::PASS_SHOW,
           "PASS_SHOW should be in transaction");
  QVERIFY2(secondResult == Enums::GIT_PULL,
           "GIT_PULL should be in transaction");
}
