...
/**
 * @brief Provide single-instance application behavior and inter-process
 * messaging using a unique key.
 *
 * This QApplication subclass detects whether another instance with the same
 * unique key is already running and enables sending and receiving textual
 * messages between instances (typically to forward command-line arguments to
 ...
 * @param uniqueKey Identifier used to scope the single-instance mechanism and
 * the local IPC resources.
 */

/**
 * @brief Query whether another instance of the application is already running.
 *
 * @returns `true` if an instance with the same unique key is already running,
 * `false` otherwise.
 */

/**
 * @brief Send a textual message to the running instance identified by the
 * unique key.
 *
 * The message is delivered to the existing instance; intended use is to forward
...
/**
 * @brief Notification emitted when a textual message from another process is
 * received.
 *
 * @param message The message payload (for example, forwarded command-line
 * arguments).
 */
