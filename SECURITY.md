...
- A folder moved or renamed carries a list bound to its old place; saving its recipients once binds it anew. The generation is monotonic per device, not a distributed sequence number: two devices can both produce the same next generation, and Git synchronisation resolves that.
