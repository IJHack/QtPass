...
- **A link inside the store is not part of it.** A store is often shared (a team's Git repository, a synced folder), and Git carries symbolic links, so a co-writer can make `git pull` create `Bank.gpg -> /elsewhere/secret.gpg` on your machine. QtPass treats a symbolic link or NTFS junction found inside the store as neither an entry, a folder nor metadata: it is skipped by every walk (re-encryption, search, staging), refused by every operation on it or on anything behind it (show, edit, add, move, copy, re-key), and a linked `.gpg-id` or `.gpg-id.sig` is not a recipient list. Deleting a link removes the link. The configured store root itself may be a link (`~/.password-store` pointing at a synced folder is a normal setup): that is your configuration, not something found inside the store.
