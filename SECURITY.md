...
- That rename replaces whatever entry is under the name by then (a link planted since the check included) and follows nothing. Re-encryption stages its output next to the entry the same way; the `.gpg-id` signature is still written in place by `gpg`.
