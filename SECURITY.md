...
- What QtPass does hold to when it adds, edits, copies or re-encrypts an entry is that `gpg` never opens a path in the store for output: it encrypts into a directory of QtPass's own outside the store, and the bytes go into the store through a file created next to the entry, written by its open handle, and the operating system's rename.
- That rename replaces a file or a link under the name by then (a link planted since the check included) as an entry and follows nothing; a directory under the name cannot be replaced by a file, so the write fails and leaves it.
- Afterwards the object under the name is opened without following and compared, by device and inode, with the file that was written: a file swapped under the temporary's name before the rename (a hard link to something of yours, which passes every check by name) is reported instead of taken for the entry.
- What is under the name then is left as it is: removing it by name could take another writer's file that landed there since. Where the system cannot give both identities the write stands rather than reporting a swap it cannot prove: on FAT and exFAT a rename moves the file ID, and neither has hard links to swap in.
- A `.gpg-id` (from the Users dialog, a new profile, or a new folder taking its parent's list) and its `.gpg-id.sig` are written the same way. On Windows a junction planted under one of these names makes the write fail rather than replace it.
- `gpg` signs the bytes QtPass just wrote, fed on its standard input (not whatever is under the file's name by the time it would read it), and writes the signature into a directory of QtPass's own, from where it is placed next to the list.
- Copying an entry reads the source as the regular file it is at the moment of the open (`O_NOFOLLOW`; on Windows the reparse point itself), not as whatever a link planted under the name since the check points to, and writes the copy the same staged way; a co-writer cannot have the bytes of a file only you can read copied into the store and committed.
- The `pass` backend runs `pass insert`, which writes the entry's path directly; a store kept with `pass` has `pass`'s semantics there.
