...
- What QtPass does hold to when it adds or edits an entry is that `gpg` never opens a path in the store for output: it encrypts into a directory of QtPass's own outside the store, and the bytes go into the store through a file created next to the entry, written by its open handle, and the operating system's rename.
- That rename replaces whatever entry is under the name by then (a link planted since the check included) and follows nothing. Re-encryption stages its output next to the entry the same way.
