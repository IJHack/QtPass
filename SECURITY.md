...
- **A signature proves authentic, not current.** An older list, genuinely signed, that still names a member since removed can be put back by the same someone. Every `.gpg-id` QtPass writes therefore starts with `# QtPass-GpgId-Generation: N`, a comment to `pass` that the signature covers.
- QtPass remembers, per list, the highest generation it has accepted or written on this device and refuses a signed list with a lower one. Saving the recipients again writes a higher generation and is the way back after a deliberate revert.
- The limits of that: it detects a rollback only on a device that has already accepted a higher generation; a device seeing the store for the first time, or one that was offline for the change, has nothing to compare against.
- The generation is monotonic per device, not a distributed sequence number: two devices can both produce the same next generation from the same list, and Git synchronisation is what resolves that. With the `pass` backend, `pass` reads the list itself and knows no generations.
